 ...
 */
	...
	public static int evaluate(Board b, int player){
		...
		int i, j;
		Piesa p;
		for (i = 1; i <= 8; i++)//percorre o tabuleiro analisando as peças e vendo se a peça é dele ou n se for recebe
			for (j = 1; j <= 8; j++){// um score se não for esse mesmo score é reduzido
 ...
 */
	public static Mutare nextMove(Board b) throws FileNotFoundException, UnsupportedEncodingException{
		int score, max = Integer.MIN_VALUE;
		int score_captura=0;
		Piesa p = null, captura = null, pmax = null;
		Position pos = null, prev, posmax = null;
		Random r=new Random();
		PrintWriter writer = new PrintWriter("media_score.txt", "UTF-8");
		int i, j, k;
		for (i = 1; i <= 8; i++)
			for (j = 1; j <= 8; j++){
				...
				if (p != null && p.Color == Game.myColor){//daca e o piesa proprie la pozitia respectiva
					...
					while (k < moves.size()){//apeleaza negamax pentru fiecare mutare
						...
						prev = p.p;
						b.move(p, pos);
						score_captura=nextMoveOponente(b);
						score = (-1) * Negamax.negaMax(b, dephtop, (-1 * Game.myColor),Integer.MIN_VALUE,-max);//adancime de 3 in afara de nivelul curent (in total 4)
						
						writer.println("MAX"+String.valueOf(max));
						writer.println(String.valueOf(score));
						score+=-1*score_captura;
						if (score == max){
							if (r.nextInt(4) == 1){
								pmax = p;
								posmax = pos;
	...
	//verificar se o oponente vai capturar alguma peça
	public static int nextMoveOponente(Board b){
		int score=0, max = Integer.MIN_VALUE;
		Piesa p = null, captura = null, pmax = null;
		Position pos = null, prev, posmax = null;
		Random r=new Random();
		int i, j, k;
		for (i = 1; i <= 8; i++)
			for (j = 1; j <= 8; j++){
				p = b.getPieceOnCoord(i, j);//pentru fiecare piesa
				if (p != null && p.Color == -1*Game.myColor){//daca e o piesa proprie la pozitia respectiva
					ArrayList<Position>moves = p.genMoves(b);//genereaza toate mutarile posibile pentru piesa
					moves = BubbleSort(p, moves, b);
					k = 0;
					while (k < moves.size()){//apeleaza negamax pentru fiecare mutare
						...
						if (captura!=null)
						{
							score+=p.getvaluepeca(pos, b);
					//		System.out.println("telluser o valor " +String.valueOf(score));
						}
						}
					}
				}
			...
			for (j = 1; j <= 8; j++){
				...
				if (p != null && p.Color == player){
					...
					while (k < moves.size()){
						...
						captura = b.getPieceOnPosition(pos);
						if (captura!=null)
							score_captura=p.getvaluepeca(pos, b);
						//score+=score_captura;
						prev = p.p;
						b.move(p, pos);
						...
						if(alfa>=betha) {
							...
							//return (alfa+player*score_captura);
							return alfa;
						}
						if(alfa>=be){
